...
n_workers = 50   # N: number of federated workers

assert tp in ('cic', 'custom'), "Wrong dataset type, choose between 'cic' and 'custom'"
assert n_workers == 50, "This script is set for 5 workers"
...
# Generate random proportions for each worker
raw_proportions = np.random.rand(n_workers)  # Random values for each worker
normalized_proportions = raw_proportions / raw_proportions.sum()  # Normalize to sum to 1

total_samples = len(train)  # Use original dataset size
minimum_rows = 100

sample_counts = [minimum_rows] * n_workers
remaining_samples = total_samples - sum(sample_counts)

proportional_counts = [int(remaining_samples * prop) for prop in normalized_proportions]


# Adjust the last client to ensure the total matches
difference = remaining_samples - sum(proportional_counts)
proportional_counts[-1] += difference

sample_counts = [sample_counts[i] + proportional_counts[i] for i in range(n_workers)]
...
for i in range(n_workers):
    ...
    n_cols = n_workers if n_workers < 50 else 50
    xlim = [0, m+(20-m%20)]
    ylim = [min(unique_codes)-1, max(unique_codes)+1]
    yticks = list(range(min(unique_codes), max(unique_codes)+1))
    yticks_labels = [mapping_inv[k] for k in range(0, max(unique_codes)+1)]
    
    plt.subplot(int(n_workers / 50)+1, n_cols, i+1)
